// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

// This was generated using spacetimedb cli version 1.3.0 (commit ).

/* eslint-disable */
/* tslint:disable */
// @ts-nocheck
import {
  AlgebraicType,
  AlgebraicValue,
  BinaryReader,
  BinaryWriter,
  ConnectionId,
  DbConnectionBuilder,
  DbConnectionImpl,
  Identity,
  ProductType,
  ProductTypeElement,
  SubscriptionBuilderImpl,
  SumType,
  SumTypeVariant,
  TableCache,
  TimeDuration,
  Timestamp,
  deepEqual,
  type CallReducerFlags,
  type DbContext,
  type ErrorContextInterface,
  type Event,
  type EventContextInterface,
  type ReducerEventContextInterface,
  type SubscriptionEventContextInterface,
} from "@clockworklabs/spacetimedb-sdk";
import { KnowledgeStatModifierDesc } from "./knowledge_stat_modifier_desc_type";
import { CsvStatEntry as __CsvStatEntry } from "./csv_stat_entry_type";

import { type EventContext, type Reducer, RemoteReducers, RemoteTables } from ".";

/**
 * Table handle for the table `staged_knowledge_stat_modifier_desc`.
 *
 * Obtain a handle from the [`stagedKnowledgeStatModifierDesc`] property on [`RemoteTables`],
 * like `ctx.db.stagedKnowledgeStatModifierDesc`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.stagedKnowledgeStatModifierDesc.on_insert(...)`.
 */
export class StagedKnowledgeStatModifierDescTableHandle {
  tableCache: TableCache<KnowledgeStatModifierDesc>;

  constructor(tableCache: TableCache<KnowledgeStatModifierDesc>) {
    this.tableCache = tableCache;
  }

  count(): number {
    return this.tableCache.count();
  }

  iter(): Iterable<KnowledgeStatModifierDesc> {
    return this.tableCache.iter();
  }
  /**
   * Access to the `secondaryKnowledgeId` unique index on the table `staged_knowledge_stat_modifier_desc`,
   * which allows point queries on the field of the same name
   * via the [`StagedKnowledgeStatModifierDescSecondaryKnowledgeIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.stagedKnowledgeStatModifierDesc.secondaryKnowledgeId().find(...)`.
   *
   * Get a handle on the `secondaryKnowledgeId` unique index on the table `staged_knowledge_stat_modifier_desc`.
   */
  secondaryKnowledgeId = {
    // Find the subscribed row whose `secondaryKnowledgeId` column value is equal to `col_val`,
    // if such a row is present in the client cache.
    find: (col_val: number): KnowledgeStatModifierDesc | undefined => {
      for (let row of this.tableCache.iter()) {
        if (deepEqual(row.secondaryKnowledgeId, col_val)) {
          return row;
        }
      }
    },
  };

  onInsert = (cb: (ctx: EventContext, row: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.onInsert(cb);
  }

  removeOnInsert = (cb: (ctx: EventContext, row: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.removeOnInsert(cb);
  }

  onDelete = (cb: (ctx: EventContext, row: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.onDelete(cb);
  }

  removeOnDelete = (cb: (ctx: EventContext, row: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.removeOnDelete(cb);
  }

  // Updates are only defined for tables with primary keys.
  onUpdate = (cb: (ctx: EventContext, oldRow: KnowledgeStatModifierDesc, newRow: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.onUpdate(cb);
  }

  removeOnUpdate = (cb: (ctx: EventContext, onRow: KnowledgeStatModifierDesc, newRow: KnowledgeStatModifierDesc) => void) => {
    return this.tableCache.removeOnUpdate(cb);
  }}
